...
class MySshClient():
    ...
    def ssh_login(self):
        try:
            ...
            self.ssh_client.connect(hostname=self.ip['remote_ips']['host'], port=self.ip['remote_ips']['port'],
                                    username=self.ip['remote_ips']['username'],
                                    password=self.ip['remote_ips']['password'])
    ...
    def remote_scp(self):
        ...
        t.connect(username=self.ip['remote_ips']['username'], password=self.ip['remote_ips']['password'])
...
def handle_oneServer(ip):
    my_ssh_client = MySshClient(ip)
...
def asynchronous_many(remoteIPs_list):
    workers = min(MAX_WORKERS, len(remoteIPs_list))  # 确定多少个线程处理
    with futures.ThreadPoolExecutor(workers) as executor:
        res = executor.map(handle_oneServer, remoteIPs_list)
    return len(list(res))
...
if __name__ == '__main__':
    ...
    asynchronous_many(remoteIPs_list)
